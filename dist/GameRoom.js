...
export class GameRoom {
    ...
    onEmpty;
    emptyTimeout = null;
    CLEANUP_DELAY = 120000; // 2 minutes
    constructor(roomCode, io, onEmpty) {
    ...
    addPlayer(id, name) {
        ...
        if (this.state.players.length >= 14 && !existing)
    ...
    nextTurn(extraTurns = 0) {
        if (this.state.turnsRemaining > 1 && extraTurns === 0) {
            ...
            do {
                ...
                if (iterations > this.state.players.length)
                    break;
    ...
    playCard(playerId, cardId, targetPlayerId) {
        ...
        if (this.state.pendingAction)
            throw new Error('Wait for current action to resolve');
        if (this.state.players[this.state.currentPlayerIndex].id !== playerId)
        ...
        if (['defuse', 'kaboom', 'nope'].includes(card.type))
            throw new Error('Cannot actively play this card');
        // Validate target for Demand and Fate Switch
        if ((card.type === 'demand' || card.type === 'fate_switch') && !targetPlayerId)
            throw new Error(`${card.type} requires a target`);
    ...
    playCombo(playerId, cardIds, targetPlayerId, namedCard) {
        if (this.state.status !== 'playing')
            throw new Error('Game is not playing');
        if (this.state.pendingAction)
            throw new Error('Wait for current action to resolve');
        ...
        if (cardsToPlay.length < 2 || cardsToPlay.length > 3)
            throw new Error('Invalid combo size');
        const allSame = cardsToPlay.every(c => c.type === cardsToPlay[0].type);
        if (!allSame)
            throw new Error('Combo cards must match');
        if (cardsToPlay.length === 3 && !namedCard)
            throw new Error('Trio requires naming a card');
        if (!targetPlayerId)
            throw new Error('Combos require a target player');
    ...
    playNope(playerId, pendingActionId, cardId) {
        ...
        if (!card || card.type !== 'nope')
            throw new Error('Valid Nope card not found in hand');
    ...
    drawCard(playerId) {
        if (this.state.status !== 'playing')
            throw new Error('Game is not playing');
        if (this.state.pendingAction)
            throw new Error('Wait for action to resolve');
        if (this.state.players[this.state.currentPlayerIndex].id !== playerId)
            throw new Error('Not your turn');
        const player = this.state.players.find(p => p.id === playerId);
        if (!player || player.isEliminated)
        ...
        this.state.drawPileCount = this.deck.count;
        if (!card)
            throw new Error('Deck empty'); // Edge case, should not happen
        if (card.type === 'kaboom') {
    ...
    insertDefusedKaboom(playerId, index, card) {
    ...
    // STATE SYNC
    getSanitizedPendingAction(action) {
        if (!action)
            return null;
